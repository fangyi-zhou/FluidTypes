...
module TyParserTests =
    ...
    let mk_test (input, ty) =
        test input {
            Expect.equal (parse input) ty (input + " is not parsed")
        }
    ...
    let ``can parse function types in unabbreviated form`` () =
        ...
        in
        let cases = [
            "(x: {v:int|true}) -> {v:int|v=x}", ``(x: {v:int|true}) -> {v:int|v=x}``
            "(x: {v:int|true}) -> (y: {v:int|true}) -> {v:int|v=x+y}", (mk_binop_type Plus)]
        in
        testList "can parse function types in unabbreviated form" (List.map mk_test cases)
