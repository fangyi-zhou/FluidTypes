...
module FreeVarTests =
    ...
    [<Property(EndSize = 10)>]
    let ``FV of lambdas does contain the bound variable`` (var : string)
        (term : Term) =
        free_var_term (Abs(var, term)) |> should not' (contain var)

    [<Property(EndSize = 10)>]
    let ``FV of base types does not contain special this`` (basety : BaseTy)
        (term : Term) =
        free_var_ty (BaseType(basety, term))
        |> should not' (contain special_this)

    [<Test>]
    let ``FV of some terms (w/o types)``() =
        let expect t vars = free_var_term t |> should equal (Set.ofList vars)
        expect x [ "x" ]
        expect (Abs("x", x)) []
        expect (Abs("x", y)) [ "y" ]
        expect (App(x, y)) [ "x"; "y" ]
    ...
    let ``FV of some types``() =
        let expect ty vars = free_var_ty ty |> should equal (Set.ofList vars)
        ...
        expect (FuncType("x", BaseType(TBool, x), BaseType(TBool, this)))
            [ "x" ]
        expect (FuncType("x", BaseType(TBool, y), BaseType(TBool, this)))
            [ "y" ]
        ...
        expect (FuncType("x", BaseType(TBool, this), BaseType(TBool, y)))
            [ "y" ]
        expect (FuncType("x", BaseType(TBool, x), BaseType(TBool, y)))
            [ "x"; "y" ]
