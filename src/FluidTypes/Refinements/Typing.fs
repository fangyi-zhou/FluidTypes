...
module Typing =
        ...
        if is_wf_type ctx ty then
            (match term with
             ...
             | t ->
                 ...
                 | Some ty_inferred -> is_subtype ctx ty_inferred ty
                 (* ty_specified *)
                 | None -> false)
        else false
    ...
    and is_wf_type (ctx : TyCtx) (ty : Ty) : bool =
        ...
        if Set.forall (fun fv -> Map.containsKey fv var_ctx) free_vars then
            (match ty with
             | BaseType(b, term) ->
    ...
    and infer_simple_type (ctx : TyCtx) (term : Term) : Ty option =
        ...
        | App(term_1, term_2) ->
            ...
            | Some(FuncType(_, t_arg, t_result)) when check_simple_type ctx
                                                          term_2 t_arg ->
                Some t_result
            | _ -> None
        | Anno(term_, ty)
